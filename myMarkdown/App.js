...
const App = () => {
  ...
  useEffect(() => {
    ...
    if(result){
      result = handleHighLight(result);
      result = handleChkBox(result);
    }
    // console.debug("resultresult",result);
  ...
  const handleHighLight = (result) => {
    ...
    if(code && code.length > 0){
      for(let i = 0;i < code.length;i++){
        ...
        let node = code.item(i).cloneNode(true).firstChild;
        while(node != null){
        ...
        str = str.replace(DIV1,"");
        str = str.replace(DIV2,"");
        code.item(i).setAttribute("msg",str);
        let lang = code.item(i).getAttribute("class");
        if(LANGUAGE.test(lang)){
          lang = lang.replace(LANGUAGE,"");
        }
        code.item(i).setAttribute("class",lang);
        console.debug("xxvvv",code.item(i).getAttribute("class"));

        let code_childN = code.item(i).childNodes;
        let len = code.item(i).cloneNode(true).childNodes.length;
        for(let j = len-1;j >= 0;j--){
          code.item(i).removeChild(code_childN.item(j));
        }
      }
    }
          ...
          renderers={
            {
              ...
              code: (htmlAttribs, children, convertedCSSStyles, passProps) => (
                <View key = {passProps.key} style={{flex: 1}}>
                  <Highlighter
                      language = {htmlAttribs.class}
                      style = {zenburn}
                    >
                    {`${(htmlAttribs.msg).replace(LESS_THAN,"<")}`}
                  </Highlighter>
                </View>
              )
            }
